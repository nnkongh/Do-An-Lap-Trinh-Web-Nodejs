  ...
  return <>
    <main className="relative flex-1 p-6 pt-28">
      ...
      <button className={`relative rounded sm:rounded-tr-none sm:rounded-br-none sm:rounded-tl-lg sm:rounded-tl-lg
        ...
        onClick={() => setFilter("borrowed")}>Sách đã trả</button>
...

        {
          booksToDisplay && booksToDisplay.length > 0 ? (
                  ...
                  <tbody>
                    {
                      booksToDisplay.map((book,index) => (
                        <tr key={index} className={(index + 1)% 2 === 0 ? "bg-gray-50" : ""}>
                          ...
                          <td className="px-4 py-2">{book.returned ? (<FaSquareCheck className="w-6 h-6"/>) :
            ...
            <h3 className="text-3xl mt-5 font-medium">
              Không {filter === "borrowed" ? "borrowed" : "overdue"} sách tìm thấy
            </h3>
            )}
    </main>
